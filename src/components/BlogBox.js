import React from 'react'
import { Link } from 'react-router-dom'
const BlogBox = (props) => { 
  return (
    <div>
        <article className="flex flex-col dark:bg-gray-200">
            <Link rel="noopener noreferrer" to="#" aria-label="Te nulla oportere reprimique his dolorum">
              <img alt="" className="object-cover w-full h-52 dark:bg-gray-500" src={require("../image1.png")} />
            </Link>
            <div className="flex flex-col flex-1 p-6">
              <Link rel="noopener noreferrer" to="#" aria-label="Te nulla oportere reprimique his dolorum"></Link>
              <Link rel="noopener noreferrer" to="#" className="text-xs tracking-wider uppercase hover:underline dark:text-violet-400">{props.item.tag}</Link>
              <h3 className="flex-1 py-2 text-lg font-semibold leading-snug">{props.item.title}</h3>
              <div className="flex flex-wrap justify-between pt-3 space-x-2 text-xs dark:text-gray-400">
                <span>{props.item.Date}</span>
                <span></span>
              </div>
            </div>
        </article>
    </div>
  )
}

export default BlogBox